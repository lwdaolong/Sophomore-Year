...
class GameState():
    ...
    def __init__(self, row: int, col: int,
                 starting_board: [[str]]):  # takes an argument of a 2d array of strings to represent
    ...
    def level_game_board(self) -> None:
        ...
        for i in range(1, self.cols + 1, 1):
    ...
    def _create_landed_faller_jewel(self,
                                    char: str) -> str:  # creates a string that symbolizes a landed faller char on the board
        return f'|{char}|'
    ...
    def create_random_faller(self):
        # TODO
        if (self.at_least_one_col_space()):
            # Check for a valid column
            l = []  # makes list of columns
            for i in range(1, len(self.game_board) - 1):
                if self.game_board[i][0] == self._create_empty_space():
                    l.append(i)  # list contains only empty columns
            random.shuffle(l)  # randomizes columns
            # Create faller with random gems
            gem_list = self.make_random_gem_list()
            self.create_faller(l[0], gem_list[0], gem_list[1], gem_list[2])
        else:
            self.game_over = True

    def make_random_gem_list(self) -> ['str']:
        gem_options = ['S', 'T', 'V', 'W', 'Y', 'X', 'Z']
        gem_list = []
        for i in range(3):
            random.shuffle(gem_options)
            gem_list.append(gem_options[0])
        return gem_list

    def at_least_one_col_space(self) -> bool:
        # TODO
        """
        Checks to see if at least one col in the game board has its top row empty
        """
    ...
    def check_diagonal_matches_down_left(self, col: int, row: int, jewel_to_check: str,
                                         control_type: int) -> int or None:
        # takes in input of col and row, to specify from what start point in the board your are starting
        # to search for matches. Also takes in jewel_to_check for reference of what to match to.
        # takes in an int for control_type
        # if control_type ==0, checks for matches and returns an int of matches(not including the starting point)
        ...
        while (col < len(self.game_board) - 1 and row < len(self.game_board[col]) - 1):
            if (len(self.game_board[col][row]) > 2 and
                    len(jewel_to_check) > 2 and self.game_board[col][row][1] == jewel_to_check[1] and
                    self.game_board[col][
                        row] !=
                    self._create_empty_space()):
                sum_matches += 1
    ...
    def check_diagonal_matches_down_right(self, col: int, row: int, jewel_to_check: str,
                                          control_type: int) -> int or None:
        # takes in input of col and row, to specify from what start point in the board your are starting
        # to search for matches. Also takes in jewel_to_check for reference of what to match to.
        # takes in an int for control_type
    ...
    def check_diagonal_matches_up_right(self, col: int, row: int, jewel_to_check: str,
                                        control_type: int) -> int or None:
        # takes in input of col and row, to specify from what start point in the board your are starting
        # to search for matches. Also takes in jewel_to_check for reference of what to match to.
        # takes in an int for control_type
