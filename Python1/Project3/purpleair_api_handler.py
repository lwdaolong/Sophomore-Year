 ...
 * ICS 32A
 * 11/13/20
 * Handles scraping data from PurpleAir API
 * @author Logan Wang
'''
...
class PurpleAirAPIHandler:

    def __init__(self, coords: (float, float), range: int, threshold: int, num_wanted: int):
        self.purpleair_url = 'https://www.purpleair.com/data.json'
        try:
            # Makes HTTP request to purpleair API
            request = urllib.request.Request(self.purpleair_url)
            ...
            if response.status != 200:
                ...
                print(self.purpleair_url)
                print('NOT 200')
            else:  # If correct response status, put url's data in a variable to convert into dictoinary later
                try:
                    self.urldata = response.read().decode(encoding='utf-8')
                    self.api_assign_attributes(response, self.urldata, coords, range, threshold,
                                               num_wanted)  # Helper method handles Format error
                finally:
                    response.close()
    ...
    def api_assign_attributes(self, http_response: 'urllib.Reponse', json_string: str, coords: (float, float),
                              range: int, threshold: int, num_wanted: int):
        """used in constructor to assign attributes and check for formatting errors.
        ...
        try:
            ...
            print('FAILED')
            print(http_response.status, end ='')
            print(self.purpleair_url)
            print('FORMAT')
    ...
    def highest_n_AQI(self, json_data: [], n: int):
        ...
        try:
            valid_locations = []
            sorted_locations_by_aqi = sorted(json_data, key=lambda x:x[1], reverse= True)  # aqi sorted by descending order
            for data in range(n):
